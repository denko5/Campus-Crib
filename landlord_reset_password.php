...
if (!isset($_SESSION['reset_email'])) {
    header("Location: landlord_forgot_password.php");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE landlord SET password = ?, failed_attempts = 0 WHERE email = ?";
    ...
    if ($stmt->execute()) {
        ...
        $message = "Password reset successful! <a href='landlord_login.php'>Login here</a>";
    } else {
...
<body>
    ...
    <div class="password-container">
            Enter new password: <input type="password" id="password" name="password" required>
            <span class="toggle-password" onclick="togglePassword()">👁️</span>
        </div>
        <button type="submit">L.Reset Password</button>
    </form>
