...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['payment_id'])) {
    ...
    if ($payment) {
        ...
        if ($action === 'approve') {
            ...
            $update_payment = "UPDATE payment SET status = 'Approved' WHERE payment_id = ?";
            $stmt = $conn->prepare($update_payment);
            $stmt->bind_param("i", $payment_id);
            $stmt->execute();
            $stmt->close();

            // Reduce vacant rooms in listing
            $reduce_vacancy = "UPDATE listing SET vacant_rooms = vacant_rooms - 1 WHERE listing_id = ? AND vacant_rooms > 0";
            $stmt = $conn->prepare($reduce_vacancy);
            $stmt->bind_param("i", $listing_id);
            $stmt->execute();
            $stmt->close();
        } elseif ($action === 'cancel') {
            // Cancel payment and increment vacant rooms if approved
            ...
            $update_payment = "UPDATE payment SET status = 'Cancelled' WHERE payment_id = ?";
            $stmt = $conn->prepare($update_payment);
            $stmt->bind_param("i", $payment_id);
            $stmt->execute();
            $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['message_id'], $_POST['reply'])) {
    ...
    if (!empty($reply)) {
        $update_reply = "UPDATE message SET reply = ? WHERE message_id = ?";
...
$query_messages = "SELECT m.message_id, m.message, m.sent_at, m.reply, s.first_name, s.last_name, s.student_id 
                   FROM message m 
...
<body>
    ...
    <div class="navbar">
        ...
        <ul>
            ...
            <li><a href="remove_vacancy.php">Remove Vacancy</a></li>
